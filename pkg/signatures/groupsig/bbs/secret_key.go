...
func (sk *SecretKey) UnmarshalBinary(in []byte) error {
	...
	if !ok {
		return fmt.Errorf("incorrect type conversion")
	}
	return nil
}
...
func (sk *SecretKey) Sign(generators *MessageGenerators, msgs []curves.Scalar) (*Signature, error) {
	...
	a, ok := b.Mul(exp).(curves.PairingPoint)
	if !ok {
...
func (sk *SecretKey) PublicKey() *PublicKey {
	publicKey, ok := sk.value.Point().Generator().Mul(sk.value).(curves.PairingPoint)
	if !ok {
		return nil
	}
	return &PublicKey{
...
type SecretKeyWithGenerators struct {
	SecretKey  *SecretKey
	Generators *MessageGenerators
}

func (*SecretKeyWithGenerators) Type() groupsig.GroupSignatureScheme {
	return groupsig.BBS
}

func (sk *SecretKeyWithGenerators) Curve() (*curves.PairingCurve, error) {
	if curve := curves.GetPairingCurveByName(sk.SecretKey.value.Point().CurveName()); curve != nil {
		return curve, nil
	}
	return nil, fmt.Errorf("invalid curve")
}

func (sk *SecretKeyWithGenerators) MessageCount() int {
	return sk.Generators.length
}

func (sk *SecretKeyWithGenerators) PublicKey() groupsig.PublicKey {
	return &PublicKeyWithGenerators{
		PublicKey:  sk.SecretKey.PublicKey(),
		Generators: sk.Generators,
	}
}

func (sk *SecretKeyWithGenerators) Sign(messages []curves.Scalar) (groupsig.Signature, error) {
	sig, err := sk.SecretKey.Sign(sk.Generators, messages)
	if err != nil {
		return nil, err
	}
	return sig, nil
...
func computeB(s curves.Scalar, msgs []curves.Scalar, generators *MessageGenerators) curves.PairingPoint {
	...
	pairingPoint, ok := pt.(curves.PairingPoint)
	if !ok {
		return nil
	}
	return pairingPoint
