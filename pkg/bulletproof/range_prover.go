...
func (prover *RangeProver) Prove(v, gamma curves.Scalar, n int, proofGenerators RangeProofGenerators, transcript *merlin.Transcript) (*RangeProof, error) {
	...
	deltayzOut, err := deltayz(y, z, n, prover.curve)
	if err != nil {
		return nil, errors.Wrap(err, "rangeproof prove")
	}
	t0 := v.Mul(z.Square()).Add(deltayzOut)
	tLinear := t1.Mul(x)
...
func getaL(v curves.Scalar, n int, curve curves.Curve) ([]curves.Scalar, error) {
	var err error
	// We need to guarantee little-endian order here, different curves have different canonical storage techniques
	// Bigint guarantees bytes to be big-endian, so we get the bytes through big int, the reverse
	var vBytes [32]byte
	copy(vBytes[:], internal.ReverseScalarBytes(v.BigInt().Bytes()))
